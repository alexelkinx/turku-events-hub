...
fetch(`${DOMAIN_NAME}/sport?id=${eventId}`)
  .then((response) => response.json())
  .then((responseData) => {
    const sport = responseData.data;
    ...
    <p class="lead card-text mt-3">Sport</p>
    <h2 class="card-title"><strong>${sport.title}</strong></h2>
    <p class="lead card-subtitle">${sport.type}</p>
  </div> `;
    cardInfo.insertAdjacentHTML("beforeend", imgHtml);
    cardBg.style.backgroundImage = `url("../assets/sport/${sport.image}.png")`;
    ...
    eventDetailsList[0].textContent = sport.type;
    eventDetailsList[1].textContent = sport.description;
    eventDetailsList[2].textContent = sport.location;
